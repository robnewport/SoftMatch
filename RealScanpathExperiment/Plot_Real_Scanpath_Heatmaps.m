...
for w = 1:15
    ...
    figure
    %tiledlayout(4,2, 'TileSpacing','loose', 'Padding','compact');
    ...
    name1Title = name1;
    name1Title = strrep(name1Title,'bluepoles','Blue Poles');
    name1Title = strrep(name1Title,'bluespot','Blue Spot');
    name1Title = strrep(name1Title,'convergence','Convergence');
    name1Title = strrep(name1Title,'turner','The Slave Ship');
    name1Title = strrep(name1Title,'starrynight','Starry Night');
    name1Title = strrep(name1Title,'pasiphae','Pasiphae');
    
    name2Title = name2;
    name2Title = strrep(name2Title,'bluepoles','Blue Poles');
    name2Title = strrep(name2Title,'bluespot','Blue Spot');
    name2Title = strrep(name2Title,'convergence','Convergence');
    name2Title = strrep(name2Title,'turner','The Slave Ship');
    name2Title = strrep(name2Title,'starrynight','Starry Night');
    name2Title = strrep(name2Title,'pasiphae','Pasiphae');

    % ScanMatch
    nexttile
    filename = sprintf('ScanMatch_%s_%s.mat', name1, name2)
    load(filename);
    h = heatmap(flipud(ScanMatchResults), 'ColorLimits', [0.0 1.0]);
    h.Title = sprintf('ScanMatch (%s vs %s)', name1Title, name2Title); 
    h.XLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    h.YLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    % end
    
    Ax = gca;
    Ax.XDisplayLabels = nan(size(Ax.XDisplayData));
    Ax.YDisplayLabels = nan(size(Ax.YDisplayData));
    Ax.XDisplayLabels([1, 53, 54, 106]) = {'_{P01} ─','^{P53} ─','_{P01} ─','^{P53} ─'};
    Ax.YDisplayLabels([1, 53, 54, 106]) = {'_{P53} ─','^{P01} ─','_{P53} ─','^{P01} ─'};
    
    exportgraphics(Ax, sprintf('Supplemental_ScanMatch_%s_%s.jpg', name1, name2) ,'Resolution',150); 
    ...
    % MultiMatch 1
    %nexttile
    filename = sprintf('MultiMatch1_%s_%s.mat', name1, name2)
    load(filename);
    h = heatmap(flipud(resultsMultiMatch1), 'ColorLimits', [0.0 1.0]);
    h.Title = sprintf('MultiMatch Vector (%s vs %s)', name1Title, name2Title); 
    h.XLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    h.YLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    % end
    ...
    exportgraphics(Ax, sprintf('Supplemental_MultiMatch_Vector_%s_%s.jpg', name1, name2) ,'Resolution',150);
    
    % MultiMatch 2
    %nexttile
    filename = sprintf('MultiMatch2_%s_%s.mat', name1, name2)
    load(filename);
    h = heatmap(flipud(resultsMultiMatch2), 'ColorLimits', [0.0 1.0]);
    h.Title = sprintf('MultiMatch Direction (%s vs %s)', name1Title, name2Title); 
    h.XLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    h.YLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    % end
    ...
    exportgraphics(Ax, sprintf('Supplemental_MultiMatch_Direction_%s_%s.jpg', name1, name2) ,'Resolution',150);
    
    % MultiMatch 3
    %nexttile
    filename = sprintf('MultiMatch3_%s_%s.mat', name1, name2)
    load(filename);
    h = heatmap(flipud(resultsMultiMatch2), 'ColorLimits', [0.0 1.0]);
    h.Title = sprintf('MultiMatch Length (%s vs %s)', name1Title, name2Title); 
    h.XLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    h.YLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    % end
    ...
    exportgraphics(Ax, sprintf('Supplemental_MultiMatch_Length_%s_%s.jpg', name1, name2) ,'Resolution',150);
    
    % MultiMatch 4
    %nexttile
    filename = sprintf('MultiMatch4_%s_%s.mat', name1, name2)
    load(filename);
    h = heatmap(flipud(resultsMultiMatch4), 'ColorLimits', [0.0 1.0]);
    h.Title = sprintf('MultiMatch Position (%s vs %s)', name1Title, name2Title); 
    h.XLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    h.YLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    % end
    ...
    exportgraphics(Ax, sprintf('Supplemental_MultiMatch_Position_%s_%s.jpg', name1, name2) ,'Resolution',150);
    
    % MultiMatch 5
    %nexttile
    filename = sprintf('MultiMatch5_%s_%s.mat', name1, name2)
    load(filename);
    h = heatmap(flipud(resultsMultiMatch5), 'ColorLimits', [0.0 1.0]);
    h.Title = sprintf('MultiMatch Duration (%s vs %s)', name1Title, name2Title); 
    h.XLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    h.YLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    % end   
    ...
    exportgraphics(Ax, sprintf('Supplemental_MultiMatch_Duration_%s_%s.jpg', name1, name2) ,'Resolution',150);
    
    % SoftMatch
    %nexttile
    filename = sprintf('SoftMatch_Matches_%s_%s.mat', name1, name2)
    load(filename);
    h = heatmap(flipud(Matches), 'ColorLimits', [0.0 10.0]);
    h.Title = sprintf('SoftMatch (%s vs %s)', name1Title, name2Title); 
    h.XLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    h.YLabel = sprintf('     %s                      %s     ', name1Title, name2Title);
    % end
    ...
    exportgraphics(Ax, sprintf('Supplemental_SoftMatch_%s_%s.jpg', name1, name2) ,'Resolution',150);
    
    %saveas(gcf, sprintf('Supplemental_%s_%s.fig', name1, name2) )
